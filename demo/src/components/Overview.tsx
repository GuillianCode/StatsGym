...
export function Overview({athlete}: {athlete: Athlete}) {
  ...
  const averages = apparatusAverages(athlete.results);
  return <section className="tab-page overview-page">
    ...
    <div className="kpi-grid">
      ...
      <article className="glass-panel kpi"><span>{athlete.discipline === 'GR' ? 'Engin' : 'Agrès'} fort</span><strong className="tag-good">{difficulty[0]?.name ?? '—'}</strong><small>ND moy. {difficulty[0]?.value.toFixed(2) ?? '—'}</small></article>
      <article className="glass-panel kpi"><span>{athlete.discipline === 'GR' ? 'Engin' : 'Agrès'} à travailler</span><strong className="tag-bad">{difficulty.at(-1)?.name ?? '—'}</strong><small>ND moy. {difficulty.at(-1)?.value.toFixed(2) ?? '—'}</small></article>
      <article className="glass-panel kpi regularity-kpi"><span>Score de régularité</span><strong>{score == null ? '—' : `${score}/100`}</strong><small>{score == null ? 'Pas assez de compétitions' : score >= 80 ? 'très régulier' : score >= 60 ? 'régulier' : score >= 40 ? 'dans la moyenne' : 'irrégulier'}</small>{score != null && <i><b style={{width: `${score}%`}} /></i>}</article>
