...
const emptyState = (): SurveyState => ({
  ...
  statsClarity: '', statsPreference: '', accessModel: '', priceRange: '', idea: '',
...
export function Survey() {
  ...
  const changeProfile = (value: string) => setState(previous => ({
    ...previous, profil: value as Profile, context: '', ratings: [0, 0, 0, 0, 0], statsClarity: '', statsPreference: '', accessModel: '', priceRange: '',
  }));
  ...
  const statsLabels = profile === 'club' ? undefined : statsQuestionLabels[profile];
  ...
  const pageValid = () => {
    ...
    if (step === 1) return state.ratings.every(Boolean) && (!statsLabels || Boolean(state.statsClarity && state.statsPreference));
    if (step === 2) {
  ...
  const payload = (): SurveyPayload => surveyPayloadSchema.parse({
    ...
    stats_preference: statsLabels ? state.statsPreference || null : null,
    club_offer: null,
  ...
  return <section className="tab-page survey-page"><article className="survey-card glass-panel">
    ...
    <div className="survey-stage">
      ...
      {step === 1 && <><h2>Évaluez les fonctionnalités de StatsGym</h2><p>Pour chaque fonctionnalité, notez son utilité de 1 (<strong>Pas du tout utile</strong>) à 5 (<strong>Indispensable</strong>).</p><div className="ratings">{profileFeatures[profile].map(([title, copy], index) => <article className="rating" key={title}><h3>{index + 1}. {title}</h3><p>{copy}</p><div className="rating-scale">{[1, 2, 3, 4, 5].map(value => <label key={value}><input type="radio" name={`rating-${index}`} checked={state.ratings[index] === value} onChange={() => setState(previous => ({...previous, ratings: previous.ratings.map((rating, ratingIndex) => ratingIndex === index ? value : rating)}))}/><span>{value}</span></label>)}</div></article>)}</div>{statsLabels && <><Field label={statsLabels.clarity}><Select value={state.statsClarity} onChange={value => set('statsClarity', value)} placeholder="Choisir une réponse" options={[['1', 'Pas du tout'], ['2', 'Plutôt non'], ['3', 'Moyennement'], ['4', 'Plutôt oui'], ['5', 'Tout à fait']]}/></Field><Field label={statsLabels.preference}><Select value={state.statsPreference} onChange={value => set('statsPreference', value)} placeholder="Choisir une réponse" options={[['simple', 'Une vue plus simple'], ['balanced', 'L’équilibre actuel me convient'], ['advanced', 'Des graphiques et analyses plus poussés']]}/></Field></>}</>}
      {step === 2 && <><h2>{accessTitles[profile]}</h2><p>Quelle proposition correspond le mieux à ce que vous attendez de StatsGym ?</p><div className="choice-list">{accessOptionsByProfile[profile].map(([value, label]) => <label className="choice" key={value}><input type="radio" checked={state.accessModel === value} onChange={() => changeAccess(value)}/><span>{label}</span></label>)}</div>{accessScale && <Field label={accessScale.prompt}><Select value={state.priceRange} onChange={value => set('priceRange', value)} placeholder={accessScale.period === 'annual' ? 'Choisir un budget annuel' : 'Choisir un budget mensuel'} options={accessScale.ranges}/></Field>}</>}
      {step === 3 && <><h2>Vos idées</h2><p>Dites-nous ce qui rendrait StatsGym plus utile ou plus accessible.</p><Field label={`${ideaLabel} (facultatif)`}><textarea rows={5} value={state.idea} onChange={event => set('idea', event.target.value)}/></Field></>}
